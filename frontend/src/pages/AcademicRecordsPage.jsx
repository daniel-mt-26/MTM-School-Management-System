...
import { getAcademicRecords, removeAcademicRecord, saveAcademicRecord } from '../api/academics'
...
function AcademicResourcePage({ resource }) {
  ...
  return <main className="student-page"><header className="student-page-header"><div><Link to="/school/academics" className="dashboard-link">Back to Academics</Link><h1>{title}</h1><p>{resource === 'enrollments' ? 'Historical enrolment records are read-only; use student transfers for class changes.' : 'Manage records for the authenticated school only.'}</p></div></header>
    ...
    {(resource === 'enrollments' || resource === 'results' || resource === 'report-cards' || resource === 'timetables') && <div className="academic-filters">{filterControl('academic_year', 'Academic Year', lookups.years, (item) => item.name)}{(resource === 'results' || resource === 'report-cards' || resource === 'timetables') && filterControl('term', 'Term', lookups.terms.filter((term) => !filters.academic_year || String(term.academic_year) === String(filters.academic_year)), (item) => `${item.academic_year_name} · ${item.name}`)}{(resource === 'enrollments' || resource === 'results' || resource === 'timetables') && filterControl('school_class', 'Class', lookups.classes, (item) => item.name)}{resource === 'results' && <>{filterControl('subject', 'Subject', lookups.subjects, (item) => item.name)}{filterControl('student', 'Student', lookups.students, (item) => `${item.display_name} · ${item.admission_number}`)}</>}{resource === 'enrollments' && <label>Current Status<select value={filters.open} onChange={(e) => setFilters((current) => ({ ...current, open: e.target.value }))}><option value="">All</option><option value="true">Open</option><option value="false">Closed</option></select></label>}</div>}
